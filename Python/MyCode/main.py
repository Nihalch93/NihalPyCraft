...
minusdays =1#easygui.enterbox("Enter Timedelta *For current date enter 0====*")
...
tickerval = getFundamentalInfo(NiftyStocks)
...
for symbol in NiftyStocks:
    ...
    try:
        ...
        bigbardf = pd.concat([bigbardf,sql(f"""select '{symbol}' as symbol,date,candle_num,sizefactor,ema9,open,close,high,low from df_15min 
                                           where substr(date,1,10)='{todaysDate}' 
                                           and upper(isbigbar)='YES' and candle_num<=2200 and candle_num>1""")]) 
        break
...
for index, row in bigbardf.iterrows():
    date = datetime.datetime.strptime(row["date"][0:19],"%Y-%m-%d %H:%M:%S")

    if row["close"]>row["open"] and row["high"]>row["ema9"]>row["low"]-1000:
        type_b_s = "b"
        target=((row["high"]-row["low"])*1.618)+row["low"]
        candle_num = row["candle_num"]
        print(f"buy order with target:: {target} on candle {candle_num}-  "+backtesting(fyers,symbol,buytime=date,type_b_s=type_b_s ,target=target,stoploss=row["low"]))

    elif row["open"]>row["close"] and row["high"]>row["ema9"]>row["low"]:
        target=row["high"]-((row["high"]-row["low"])*1.618)
        type_b_s = "s"
        candle_num = row["candle_num"]
        print(f"sell order with target:: {target} on candle {candle_num}:: "+backtesting(fyers,symbol,buytime=date,type_b_s=type_b_s ,target=target,stoploss=row["low"]))
